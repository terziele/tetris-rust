...
const LEVEL_TIME: [u32; 10] = [1000, 850, 700, 600, 500 ,400, 300, 250, 221, 190];
...
fn handle_events(tetris: &mut Tetris, quit: &mut bool,
                 timer: &mut SystemTime, event_pump: &mut sdl2::EventPump) -> bool {
...
fn save_highscores_and_lines(highscores: &[u32], number_of_lines: &[u32]) -> bool {
    ...
    write_into_file(&format!("{}\n{}\n\n", s_highscores, s_number_of_lines),HIGHSCORE_FILE)
        .is_ok()
}
...
fn print_game_information(tetris: &Tetris) {
    ...
    if let Some((mut highscore, mut lines_sent)) = load_highscores_and_lines() {
        new_highest_score = update_vec(&mut highscore, tetris.score);
    ...
    println!("Game over...");
    println!("Score:   {}{}", tetris.score, if new_highest_score {"[NEW HIGHSCORE]"} else {""});
    ...
    match timer.elapsed() {
        Ok(elapsed) => {
            let millis = elapsed.as_secs() as u32 * 1000 + elapsed.subsec_nanos()
                / 1_000_000;
            millis > LEVEL_TIME[tetris.current_level as usize -1]
        },
        Err(_) => false,
    }
}
...
fn create_texture_rect<'a>(canvas: &mut Canvas<Window>, 
                           ...
                           size_width: u32, size_height: u32) -> Option<Texture<'a>> {
    if let Ok(mut square_texture) = texture_creator.create_texture_target(None, size_width,size_height) {
...
fn main() {
    ...
    let mut font = ttf_context.load_font("assets/lucida.ttf", 128).expect("Unable to load ttf font");
    ...
    let mut canvas = window.into_canvas()
        ...
        .present_vsync()
        .build()
        .expect("Couldn't get a window's canvas");
    ...
    loop {
        ...
        let rendered_text = create_texture_from_text(&texture_creator, &font, "test", 255,255,255)
            .expect("Cannot render text");
        canvas.copy(&rendered_text, None, Some(Rect::new(width as i32 - 40, 0,40,30)))
        ...
        //we need to draw the game map here
